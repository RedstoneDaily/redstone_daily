...
2. **生成密钥并授权后端仓库：**
	...
	1. **生成密钥对：**
		...
		然后它会跟你进行对话，
		首先询问你设置文件名和路径，建议放到`~/.ssh/<key-name>`（自己起个名字，不带后缀）
		然后询问你设置passphrase密码，留空直接回车，再次输入也留空回车
		然后就可以生成密钥了~
...
2. 在`~/redstone_daily`里运行以下命令克隆后端仓库：
...
3. 创建`frontend`文件夹，联系Static运行Github Action把前端部署进去
